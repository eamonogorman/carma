PRO make_fig2
;
; Note, running in interactive first (comment out set_plot), 
;then run contour line on its own. Finnaly uncomment and run set_plot. 
;Contour willstick on whatever position you 1st give it. Very strange.
;
...
;###################################### chan 34 ################################
set_plot, 'ps'
device, file='test__34.ps',/color, bits=24, /encapsulate, xsize=1.5, ysize=1.5, /inches
make_ct, ncolors=3
imag=b[130:320,130:320,34]
tvscl, imag
;
PLOTS, 0.10*321, 0.11*321, thick=2, color=1
PLOTS, 0.10*321+60*321*(0.00311526), 0.11*321, /CONTINUE, thick=2, color=1
xyouts,30,20,'6 arcsec', charthick=2.0, charsize=0.7, color=1
...
xyouts,110,170,'-9.0 km s!u-1!n', charthick=2.0, charsize=0.7, color=1
DEVICE, /CLOSE
set_plot, 'x'
;###################################### chan 35-40 ################################
For i=35,41 do begin
set_plot, 'ps'
device, file='test__'+string(i,format='(g0.3)')+'.ps',/color, bits=24, /encapsulate, xsize=1.5, ysize=1.5, /inches
make_ct, ncolors=3
;
;device, decomposed=0
;
imag=b[130:320,130:320,i]
tvscl, imag
;
...
xyouts,110,170,vel[i-34]+' km s!u-1!n', charthick=2.0, charsize=0.7, color=1	;
...
CONTOUR, imag, LEVELS = V, C_LINESTYLE = (V LT 0.0) , XSTYLE = 1, YSTYLE = 1, /noerase,c_colors=[3,0,0,0], POSITION=[0.0, 0.0, 1.0, 1.0], c_thick=1.5, /overplot
xyouts,110,170,'-17.9 km s!u-1!n', charthick=2.0, charsize=0.7, color=1
DEVICE, /CLOSE
set_plot, 'x'
;
End
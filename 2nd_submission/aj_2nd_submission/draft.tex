...
\altaffiltext{6}{Max-Planck-Institut für Radioastronomie, Auf dem Hügel 69, 53121 Bonn, Germany}
...
The scaled SOFIA-GREAT emission line profile binned to $1.3\>{\rm km\>s}^{-1}$ is shown in Figure \ref{fig:fig8} along with the CARMA C configuration CO(\textit{J} = 2-1) profile which spatially filters the S2 contribution. This figure shows that the $\textit{J} = 12-11$ profile reflects the slower moving S1 flow with a width of $\sim \pm 7.5\>{\rm km\>s}^{-1}$ approximately centred on the stellar rest frame. This suggests that the $\textit{J} = 12-11$ emitting plasma is not associated with the faster S2 component and is likely associated with the higher excitation S1 plasma. Owing to uncertainties in the pointing accuracy during our GREAT observation we defer a discussion of the fluxes to a later time. A visual inspection of archival Herschel spectra also reveals the diminishing nature of the S2 line contribution relative to the S1 with increasing rotational J values.

\begin{figure*}[hbt!]
%[trim=40pt 0pt 50pt 0pt, width=8.0cm, height=7.5cm]
\centering
\includegraphics[trim=60pt 0pt 0pt 0pt, scale=0.52]{f19.eps}
